...
dose.conc.cov <-
  ...
  mutate(
    conc = dose / clearance * exp(rnorm(dose, 0, logsd.exposure)),
...
exposure.response.sample.with.cov <-
  dose.conc.cov %>%
  mutate(
    cov3num = as.numeric(as.character(cov3)),
    resp =
...
exposure.response.sample.binary <-
  tibble(conc = conc) %>%
  mutate(
    sex_num = rep(c(0, 1), length.out = length(conc)),
    sex = ifelse(sex_num == 1, "female", "male")
  ) %>%
  mutate(
    y_logit = fun_emax(conc, e0 = -1.5, emax = 3, ec50 = 100),
